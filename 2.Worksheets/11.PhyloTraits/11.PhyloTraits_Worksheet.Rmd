...
> ***Answer 1***: The afa file is upper case, while the fasta file is lower case. Also, the afa file has "-" to indicate breaks in the sequence, while the fasta file does not have breaks. The afa file has N's, while the fasta file does not.
...
```{r}
setwd("~/GitHub/QB2019_Peckenpaugh/2.Worksheets/11.PhyloTraits")
read.aln <- read.alignment(file = "./data/p.isolates.afa", format = "fasta")
p.DNAbin <- as.DNAbin(read.aln)
window <- p.DNAbin[, 100:500]
image.DNAbin(window, cex.lab = 0.50)
grid(ncol(window), nrow(window), col = "light grey")
```
...
> ***Answer 2a***:  Most sequence reads are around 700-900 bp long, while Methanosarcina is more like 1500 bp long.
> ***Answer 2b***:  There are regions from ~200-400 and ~500-700 bp that have sites for most or all of the species, appear to be overall conserved but have sufficient variation to see differences.
...
> ***Answer 3***:  It is likely not as accurate or robust as a maximum likelihood or bayesian tree, but it requires significantly less computational power, so it can be done quickly. It is also not a statistical method.
...
par(mar = c(1, 1, 2, 0))
plot.phylo(raw.rooted, type = "phylogram", use.edge.length = FALSE, direction = "right", show.tip.label = TRUE, adj = 0.5, cex = 0.6, label.offset = 2, main = "Raw")
par(mar = c(1, 0, 2, 1))
plot.phylo(F84.rooted, type = "phylogram", use.edge.length = FALSE, direction = "left", show.tip.label = TRUE, adj = 0.5, cex = 0.6, label.offset = 2, main = "F84")
```
...
> ***Answer 5a***:   The maximum likelihood tree is quite different from the neighbor-joining tree, which makes sense since neighbor joining trees only use a distance matrix, while maximum likelihood take specific nucleotide states into account. Maximum likelihood is also less sensitive to the substitution model used.
> ***Answer 5b***:   Bootstraps are a statistical method used with maximum likelihood to determine how confident we should be in the placement of each branch.
> ***Answer 5c***:   Higher boostrap values suggest higher support.
> ***Answer 5d***:   The internal nodes have the lowest support overall, though some other branches have very low support (e.g., WG42, LL43F).
> ***Answer 5e***:   No, we probably should not trust these branches.
...
> ***Answer 6a***:   My hypothesis is that there is a cost to being able to use a lot of different forms of phosphorus.
> ***Answer 6b***:   I would expect to see lower maximum growth rates with higher niche breadth, and vice versa.
...
read.aln <- read.alignment(file = "./birds.afa", format = "fasta")
b.DNAbin <- as.DNAbin(read.aln)
window <- b.DNAbin[, 100:500]
image.DNAbin(window, cex.lab = 0.50)

seq.dist.k <- dist.dna(b.DNAbin, model = "K80", pairwise.deletion = FALSE)

nj.tree <- bionj(seq.dist.k)
outgroup <- match("Buteo", nj.tree$tip.label)
nj.rooted <- root(nj.tree, outgroup, resolve.root = TRUE)
par(mar = c(1,1,2,1) + 0.1)
...
> This tree seems to align quite well with my (very limited) knowledge of these species! The two species in the Vireo genus group together. Also, Mniotilta and Dendroica are both warblers. Turdus and Catharus are both thrushes. One thing that could improve this tree would be to do more research to determine the most appropriate outgroup, or to add something that I know is an outgroup. I did some research and thought Buteo (hawks) might be appropriate, but I'm not totally sure. Also, in terms of future analyses, it might be worth looking into more bird sequences for this gene, though I'm not sure how much more is out there. There are more species in our data that are not included in this tree.
