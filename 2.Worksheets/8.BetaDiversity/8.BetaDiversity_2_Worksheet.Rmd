...
> ***Answer 1***: Based on the PERMANOVA, habitat quality appears to significantly predict the distribution of fish species. While there are not very many indicator species for only one group of habitat quality, there are several species with high indicator value for low and medium quality sites combined. Overall, quite a few species show preference for habitat quality groups. There are more species that show preference for low/medium quality habitats, though the preference is strongest in the few species that prefer high quality habitat. These analyses seem pretty consistent to me.
...
> ***Answer 2***:  The Mantel test suggests that 60.4% of the variation in fish distribution is explained by the stream environmental condition data provided, and this result is statistically signifcant (p = 0.001). I think this evidence supports my hypothesis that stream quality influences fish communities.
...
> ***Answer 3***: Dissolved oxygen, biological demand for oxygen, and nitrates.
...
> ***Answer 4***: 13% of the variance in doubs fish diversity is explained by environmental variables, after controlling for spatial variation. 24% of the variance is explained by spatial position, after controlling for environmental variation across space. 37% of variance in fish diversity is explained by spatially structured environmental variation. 27% of the variance is not explained by these variables.
...
jb <- read.table("JellyBeans.txt", sep = "\t", header = TRUE)

# Rearrange rows by group
jb <- jb[c(1:3, 6, 8, 4:5, 7, 9), ]

group <- c(rep("A", 5), rep("B", 4))
jb.num <- jb[, 3:30]

# PERMANOVA
adonis(jb.num ~ group, method = "bray", permutations = 999)
```

> ***Answer 1***: The p-value of this PERMANOVA is 0.057, which is not statistically significant considering a standard $\alpha$ = 0.05. It is close to significance, however, this result might suggest that deterministic processes generally play a larger role in estimates of biodiversity than stochastic effects. I'm sure it varies by dataset, though.


```{r}
setwd("~/Box Sync/2018-2019/Spring classes/Quantitative biodiversity")
...
plot(bird.pcoa$points[ ,1], bird.pcoa$points[ ,2], ylim = c(-0.2, 0.7),
...
> ***Answer 2***: Based on my PCoA, while the bird species do not make clear clusters by mortality class of hemlock, there does appear to be a gradient from low to high in PCoA 1. However, PCoA 1 only explains 21.2% of the variance in bird species presence. I am hypothesizing that certain bird species may prefer certain habitats, based on the mortality class of hemlock. I am using the phi coefficient of association to test this. This test showed 6 species with statistically significant species-site group associations: one species (Brown creeper) is associated with Hi-A, two species (Eastern wood peewee, Hooded warbler) are associated with Hi-B, two species (Black-throated green warbler, Blackburnian warbler) are associated with Low mortality, and one species (Tufted titmouse) is associated with the groups Hi-A, Hi-B, and Medium. This result suggests that certain bird species may preferentially associate with either the presence or absence of hemlock in their habitat.
